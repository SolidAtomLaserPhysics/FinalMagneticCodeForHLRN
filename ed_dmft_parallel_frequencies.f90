program lisalanc
    ...
    real(dp),  parameter :: BField = dfloat(p)/dfloat(L)
    ...
    complex(dp) G0wpartMat(0:Iwmax,1:L,1:L), G0wmat(0:Iwmax,1:L,1:L)  
!   matrix & friends
         ...
         if (lattice_type.eq.4) then
            ...
            if (lattice_type.eq.3) then
               ...
               if (lattice_type.eq.2) then   
                 write(6,*) 'face-centered cubic lattice'
               else
                  if (lattice_type.eq.5) then
                     write(6,*) '2D-simple cubic in magnetic field'
                  else
...
!           not converged now
 ...
 68      if (myid.eq.0) then
         ...
         elseif (lattice_type.eq.5) then        
            if (myid.eq.0) then      
               write(6,*) 'for the 2D-square in the magnetic case' 
            endif
         ...
         if (lattice_type.eq.5) then
            if (myid.eq.0) then
               ...
               do i=0,Iwmax
                  G0w(i)=dcmplx(0.0d0,0.0d0)
               enddo
   
               do i=0,Iwmax
                  do j=1,L
                     G0w(i)=G0w(i)+G0wmat(i,j,j)/dfloat(L)
                  enddo
               enddo
